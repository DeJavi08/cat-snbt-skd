            ...
            <div className="space-y-3.5" id="multiple-choice-block">
              {soal.pilihan &&
                Object.entries(soal.pilihan).map(([key, value]) => {
                  ...
                  // Retrieve TKP point values if applicable (to expose in review)
                  const poinVal = soal.poin ? (soal.poin as Record<string, number>)[key] : null;
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* TKP point weight or answer correctness check indicators in Review/Kuis Mode */}
                      {(!isUjian && !isKuis || (isKuis && hasAnswered)) && (
                        <div className="shrink-0 flex items-center gap-2 self-center">
                          {poinVal !== null && (
                            <span className="inline-flex text-[10px] md:text-xs font-bold px-2 py-1 bg-amber-50 dark:bg-amber-950/45 border border-amber-200 dark:border-amber-900/60 text-amber-705 dark:text-amber-400 rounded-lg">
                              Poin: {poinVal}
                            </span>
                          )}
                          {isSecorrectOption && (
