...
export function hitungSkorSKD(
  ...
  soalList.forEach((soal) => {
    ...
    if (category === "Tes Karakteristik Pribadi (TKP)") {
      // TKP can have varying points 1 to 5
      if (soal.poin) {
        ...
        if (ans && typeof ans === "string") {
          if (ans === soal.jawaban) {
            ...
            // Simulated stable graded points (1-4) based on chosen letter to maintain consistency
            const code = ans.toUpperCase().charCodeAt(0) || 65;
            earned = ((soal.nomor + code) % 4) + 1; // ranges 1 to 4
          }
        } else {
          earned = 0;
        }
